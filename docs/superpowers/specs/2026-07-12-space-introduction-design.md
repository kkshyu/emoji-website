...
範圍：前台空間頁、後台網站內容／上傳圖、導覽與廢除獨立 `/menu`、CIS 對齊
...
- 空間頁視覺與語氣遵循公開 CIS（余白 · Edition 2026 v1.0）。
...
空間頁（含三語）必須符合公開《余白 · 言文字 CIS》與全站 token，不另開色票或字體。

| 面向 | 要求 |
|------|------|
| 色彩 | 墨 `#1B1A17`、紙白灰階為主；強調黃 `#FFDE34` 每畫面至多一擊、只當底，墨字壓上。沿用 `/style.css` CSS 變數。 |
| 字體 | 標題 Noto Serif TC；拉丁／樓層號／價位 Cormorant Garamond；內文與介面 Noto Sans TC。 |
| 語氣與命名 | 短句、余白、不急推銷。母品牌「言文字｜台灣人才聚落」；一樓餐飲「Emoji · Café & Bar」。 |
| 法遵用詞 | 二樓禁用住宿、入住、過夜、旅館、hotel、共居、居住等字詞；描述為休憩與會員服務。 |
| 版面 | 保有余白與節奏；不堆卡片牆、不裝飾性紫光／多層陰影；標誌若出現須用 `/cis/assets/` 素材、不改色變形。 |
| 檢核 | 實作完成前對空間頁跑 CIS 三秒檢核：焦點、留白、黃只一擊、手機仍有余白。 |

Seed 與後台可編正文亦須遵守同一語氣與禁用詞；頁面專屬 CSS 若偏離 CIS，以全站 token 與本表為準修正。
...
- 來源：營運計畫書顧客向樓層說明（機能、動線、氛圍）；**不含**貸款、內部敏感數字；語氣與禁用詞對齊 CIS。
...
| CIS | 空間頁對齊余白 CIS（色／字／語氣／禁用詞／三秒檢核） |
