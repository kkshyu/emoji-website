...
function verifyAccessToken(token, secret, { now = Date.now() / 1000 | 0 } = {}) {
  if (typeof token !== 'string' || token.length > 8192 || !secret || token.indexOf('.') < 0) return null;
  ...
  if (!p || !Number.isFinite(p.exp) || now >= p.exp) return null;
  if (!Number.isFinite(p.iat) || p.iat > now || p.exp <= p.iat) return null;
