...
function s3Client() {
  ...
  return new Minio.Client({
    endPoint: u.hostname, port: Number(u.port) || (u.protocol === 'https:' ? 443 : 80),
...
async function renderPostImages(post, deps) {
  ...
  try {
    ...
    const outDir = useS3 ? require('os').tmpdir() : deps.uploadDir;
    const mc = useS3 ? s3Client() : null;
    for (let i = 0; i < post.pages.length; i++) {
      ...
      await page.screenshot({
        type: 'jpeg', quality: 92, path: localPath,
        clip: { x: 0, y: 0, width: info.w, height: info.h },
      });
      if (useS3) {
        await mc.fPutObject(S3_BUCKET(), `posts/${file}`, localPath, { 'Content-Type': 'image/jpeg' });
...
module.exports = { normalizePage, checkBanned, checkStyle, renderPostImages, publishPost, publishDue, refreshToken, getToken, bannedList, aiPhraseList };
