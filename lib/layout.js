...
const TRAILING_SLASH = new Set(['cis', 'menu']);
...
function localePaths(reqPath) {
  ...
  // member：僅中文頁；英／日切回各語系首頁
  if (slug === 'member') {
    return { lang: 'zh', slug: 'member', zh: '/member', en: '/en/', ja: '/ja/' };
  }
  // menu：僅中文頁；三語皆指 /menu/
  if (slug === 'menu') {
    return { lang, slug: 'menu', zh: '/menu/', en: '/menu/', ja: '/menu/' };
  }
  // space：僅中文頁；三語皆指 /space
  if (slug === 'space' || slug === 'space.html') {
    return { lang, slug: 'space', zh: '/space', en: '/space', ja: '/space' };
  }
...
function composeLayout(html, reqPath) {
  ...
  const vars = {
    ...
    NAV_MENU_CURRENT: meta.slug === 'menu' ? CURRENT : '',
