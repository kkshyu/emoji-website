...
const SPACE_SEED = {
  space_1f_zh: `白天是明亮的日式簡餐與手沖咖啡，設外帶視窗；入夜燈光轉暖，成為深夜食堂與酒吧。這裡是聚落的入口，很多相遇從這一杯開始。

- 約 25 坪；內用約 20–30 席、長吧檯、外帶視窗
- 白日咖啡，入夜為酒
- 不用會員，一杯咖啡就是入場券`,
  space_1f_en: `By day: Japanese set meals and pour-over coffee, with a takeout window. After dark the lights warm into a late-night shokudō and bar. This is the doorway to the hub, where the meetings begin.

- About 25 ping; 20–30 seats, long counter, takeout window
- Open to everyone: a coffee is your ticket in`,
  space_1f_ja: `昼は明るい日本式の定食とハンドドリップ。テイクアウト窓口あり。夜は灯りを落とし、深夜食堂とバーへ。ここはタレントハブの入口。出会いは一杯から始まります。

- 約25坪、20〜30席、ロングカウンター
- 会員でなくても、一杯のコーヒーから`,
  space_2f_zh: `會員專屬樓層：人臉辨識進出。24 小時看書休憩席（含獨立膠囊空間）、淋浴、電動漫畫娛樂室（Switch、漫畫、桌遊、麻將），也可包場聚會。

- 約 28 坪；休憩與娛樂分區、隔音加強
- 席位掃 QR 自助登記計時；非密閉、不可上鎖
- 僅供會員休憩與活動`,
  space_2f_en: `Members-only floor with face-entry access. 24-hour reading and rest seats (with private capsule pods), showers, and a game and manga room (Switch, manga, board games, mahjong). Also bookable for gatherings.
...
- Member rest and recreation only`,
  space_2f_ja: `会員専用フロア。顔認証で入退室。24時間の読書・休憩席（個室カプセルスペース付き）、シャワー、ゲーム・マンガルーム（Switch・マンガ・ボードゲーム・麻雀）。貸切も可。

- 約28坪。休憩と娯楽を分け、遮音を強化
- 席ごとにQRでチェックインし、時間を記録。密閉不可・施錠不可
- 会員の休憩と交流のための空間`,
  space_3f_zh: `老屋斜屋頂、挑高六米的木樑下，是共享辦公與活動場。桌椅可移，白天專心工作，晚上變成講座與社群活動。
