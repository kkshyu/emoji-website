...
(function () {
  ...
  function injectCSS() {
    ...
    const css = `
    .ig-wrap{display:grid;grid-template-columns:340px 1fr;gap:24px;align-items:start}
    @media(max-width:900px){.ig-wrap{grid-template-columns:1fr}}
    ...
    .ig-previewbox{background:var(--paper-deep);border:1px solid var(--line);border-radius:var(--radius);padding:22px;display:flex;flex-direction:column;align-items:center;gap:16px}
    .ig-stage{overflow:hidden;box-shadow:var(--shadow);background:#fff}
    ...
    .igp-body{flex:1;display:flex;flex-direction:column;padding:76px 92px 80px;}
  ...
  function renderForm() {
    ...
    box.innerHTML = F[state.type]();
    // 值變更 → 只更新預覽（不重繪表單，保留游標）
    box.querySelectorAll('[data-k]').forEach(inp => inp.addEventListener('input', e => {
  ...
  function sizeStage() {
    const { w, h } = DIMS[state.format];
    const avail = Math.min(stage.parentElement.clientWidth - 4, 460);
    const scale = avail / w;
    stage.style.width = w * scale + 'px';
    stage.style.height = h * scale + 'px';
    const node = stage.firstElementChild;
    if (node) { node.style.transformOrigin = 'top left'; node.style.transform = `scale(${scale})`; }
  }
  ...
  async function download() {
    ...
    try {
      ...
      const name = state.type === 'product' ? state.p_zh : state.type === 'event' ? state.e_title : '金句';
      const a = document.createElement('a');
      a.href = url; a.download = `言文字_IG_${name}_${state.format === 'square' ? '方形' : '直式'}.png`; a.click();
      toast('已下載 PNG');
    ...
    return `<div class="ig-wrap">
      <div class="ig-form">
        ...
        <div class="ig-drop" id="ig-drop"><span id="ig-drop-t">上傳照片 · 點此或拖曳</span>
          <input type="file" accept="image/*" id="ig-file" hidden></div>
      ...
      <div class="ig-previewbox">
        <div style="width:100%;display:flex;justify-content:center"><div class="ig-stage" id="ig-stage"></div></div>
